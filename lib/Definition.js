...
class Definition {
  ...
  addOptionsToConfig(options) {
    ...
    for (const attributeName in options) {
      const isAttribute = this.model.attributes? this.model.attributes[attributeName]: this.model.rawAttributes[attributeName];
    ...
    while (tries < creationRetries && !out) {
      try {
        ...
        if (err.name === 'SequelizeUniqueConstraintError') {
          debug('factory')(`factory retry ${tries}: ${err.parent.detail}`);
        }
      }
    }
    if (!out) { debug('factory')(reason); throw reason; }
    return out;
  }
