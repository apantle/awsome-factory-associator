...
class Definition {
  constructor(factory, name, model) {
    ...
    if (! this.creationRetries) {
      this.creationRetries = typeof sails === 'undefined'
        ? defaultRetries
  ...
  assoc(as, factoryName, options, associationSequelizeOptions) {
    associationSequelizeOptions =
      associationSequelizeOptions || this.getValidAssociation(as, 'BelongsTo');
    const factory =
      factoryName ||
      (this.config.assoc[as] ? this.config.assoc[as].factoryName : null);
  ...
  assocMany(as, factoryName, optionsArray, associationSequelizeOptions) {
    associationSequelizeOptions =
      associationSequelizeOptions ||
      this.getValidAssociation(as, 'BelongsToMany');
    const factory =
      factoryName ||
      (this.config.assocMany[as]
        ? this.config.assocMany[as].factoryName
        : null);
  ...
  assocAfter(as, factoryName, options, associationSequelizeOptions) {
    associationSequelizeOptions =
      associationSequelizeOptions || this.getValidAssociation(as, 'HasOne');
    const factory =
      factoryName ||
      (this.config.assocAfter[as]
        ? this.config.assocAfter[as].factoryName
        : null);
  ...
  assocManyAfter(as, factoryName, options, associationSequelizeOptions) {
    associationSequelizeOptions =
      associationSequelizeOptions || this.getValidAssociation(as, 'HasMany');
    const factory =
      factoryName ||
      (this.config.assocManyAfter[as]
        ? this.config.assocManyAfter[as].factoryName
        : null);
    ...
    while (tries < this.creationRetries && !out) {
      try {
        ...
        if (err.name === 'SequelizeUniqueConstraintError') {
          debug('awsome-factory')(`factory retry ${tries}: ${err.parent.detail}`);
        }
      }
    }
    if (!out) {
