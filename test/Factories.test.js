...
describe('Factories test', function () {
  describe('Creation', function () {
    ...
    it('should error using invalid factory name', () => {
      try {
        return factory.create('Wrong').then(() => {
          true.should.be.false();
        });
      } catch (err) {
        should.exist(err);
  ...
  describe('Attributes', function () {
    it('should create using function as attribute', () => {
      factory
        .define('passengerFunction', Passenger)
        .attr('name', faker.lorem.word);
      return factory
        .create('passengerFunction')
        .then((passenger) => {
          (passenger instanceof Passenger).should.be.true();
    ...
    it('should create using function arready called as attribute', () => {
      ...
      return factory.create('saleCancelled').then((sale) => {
    ...
    it('should create using null as attribute', () => {
      factory
        .define('saleNotCancelled', Sale)
        .attr('total', faker.random.number)
        .attr('cancelled_at', null);
      return factory.create('saleNotCancelled').then((sale) => {
    ...
    it('should overwrite value incremented', () => {
      return factory
        .create('passengerAutoIncrement', { name: 'nameString' })
        .then((passenger) => {
          (typeof passenger.name).should.eql('string');
    ...
    it('should share increment with children and alter attributes', () => {
      factory
        .define('ticketIncrementCodeChild', Ticket)
        .parent('ticketIncrementCode')
        .attr('seat', 'X');
      return factory.create('ticketIncrementCodeChild').then((ticket) => {
        (typeof ticket.price).should.eql('number');
  ...
  describe('Assoc', function () {
    it('should define a ticket asociated to a sale', () => {
      ...
      factory.definitions.ticketDef.config.assoc.Sale.should.have.property(
        'factoryName',
        'saleDef'
      );
      factory.definitions.ticketDef.config.assoc.Sale.should.have.property(
    ...
    it('should get associations from parent', () => {
      factory.define('ticketAFree').parent('ticketA').attr('price', 0);

      factory.definitions.ticketAFree.config.assoc.should.have.property('Sale');
      factory.definitions.ticketAFree.config.assoc.Sale.should.have.property(
        'factoryName',
        'saleA'
      );
      factory.definitions.ticketAFree.config.assoc.Sale.should.have.property(
    ...
    it('should get associations from parent and overwrite associations', () => {
      factory.define('saleFree', Sale).attr('total', 0);
      factory
        .define('ticketAFreeSale')
        .parent('ticketAFree')
        .assoc('Sale', 'saleFree');

      factory.definitions.ticketAFreeSale.config.assoc.should.have.property(
        'Sale'
      );
      factory.definitions.ticketAFreeSale.config.assoc.Sale.should.have.property(
        'factoryName',
        'saleFree'
      );
      factory.definitions.ticketAFreeSale.config.assoc.Sale.should.have.property(
    ...
    it('should create a belongs to association', () => {
      return factory.create('ticketA').then((ticket) => {
    ...
    it('should create a belongs to association with save option', () => {
      return factory
        .create('ticketA', { Sale: { $: 'saleSaved' }, $: 'ticketCreated' })
        .then((ticket) => {
          ticket.should.have.property('seat', '22A');
          ticket.should.have.property('Sale');
    ...
    it('should create a belongs to association overwriting the factory name', () => {
      factory.define('saleFacName', Sale).attr('total', 9);
      return factory
        .create('ticketA', { Sale: { _factoryName: 'saleFacName' } })
        .then((ticket) => {
          ticket.should.have.property('seat', '22A');
          ticket.should.have.property('Sale');
  ...
  describe('AssocMany', function () {
    it('should define a salseman asociated to many stores', () => {
      ...
      factory
        ...
        .attr('name', 'Sus')
        .assocMany('StoreHired', 'storeDef', [
          { city: 'Paris' },
          { city: 'Barcelona' }
        ]);

      const assocManyConfig =
        factory.definitions.salesmanWithStores.config.assocMany;
      assocManyConfig.should.have.property('StoreHired');
      assocManyConfig.StoreHired.should.have.property(
        'factoryName',
        'storeDef'
      );
      assocManyConfig.StoreHired.should.have.property('options', [
        { city: 'Paris' },
        { city: 'Barcelona' }
      ]);
    ...
    it('should create a belongs to many association', () => {
      return factory.create('salesmanA').then((salesman) => {
    ...
    it('should create a belongs to many overwriting factory name', () => {
      factory.define('storeNew', Store).attr('city', 'Berlin');
      return factory
        .create('salesmanA', { StoreHired: [{ _factoryName: 'storeNew' }] })
        .then((salesman) => {
          salesman.should.have.property('name', 'Sus');
          salesman.should.have.property('StoreHired');
          salesman.StoreHired.should.have.length(1);
    ...
    it('should create a belongs to many using size and save it', () => {
      return factory
        .create('salesmanA', {
          name: faker.name.firstName,
          StoreHired: { city: 'LA', _size: 2, $: 'store-' }
        })
        .then((salesman) => {
          salesman.should.have.property('name');
          salesman.should.have.property('StoreHired');
    ...
    it('should save associated instance in same object and use it', () => {
      factory.define('storeRandom', Store).attr('city', faker.address.city);
      factory
        .define('salesmanWithRandomStores', Salesman)
        .attr('name', 'Sus')
        .assocMany('StoreHired', 'storeRandom', [
          { $: 'store1' },
          { city: '$store1.city', $: 'store2' }
        ]);
      return factory
        .create('salesmanWithRandomStores', { name: 'Ale' })
        .then((salesman) => {
          ...
          salesman['$'].store2.should.have.property(
            'city',
            salesman['$'].store1.city
          );
        });
    });
  });
  ...
  describe('hasOne', function () {
    it('should define a ticket asociated to one discount', () => {
      ...
      const assocAfterConfig =
        factory.definitions.ticketFactDiscount.config.assocAfter;
      assocAfterConfig.should.have.property('MainDiscount');
      assocAfterConfig.MainDiscount.should.have.property(
        'factoryName',
        'discountFact'
      );
      assocAfterConfig.MainDiscount.should.have.property('options', {
        percentage: 50
      });
      assocAfterConfig.MainDiscount.should.have.property(
        'foreignKey',
    ...
    it('should create a has one association', () => {
      return factory
        .create('ticketWithDiscount', { price: 100 })
        .then((ticket) => {
    ...
    it('should create a has one association with options', () => {
      return factory
        .create('ticketWithDiscount', { MainDiscount: { percentage: 80 } })
        .then((ticket) => {
          ticket.should.have.property('MainDiscount');
    ...
    it('should create a has one association with diferent factory', () => {
      ...
      return factory
        .create('ticketWithDiscount', {
          MainDiscount: { _factoryName: 'discountFull' }
        })
        .then((ticket) => {
          ticket.should.have.property('MainDiscount');
      ...
      return factory
        ...
        .catch((err) => {
          err.message.should.equal(
            'Ids can not be passed in MainDiscount since it is a HasOne or HasMany association.'
          );
        });
    });
    ...
    it('should save has one association', () => {
      return factory
        .create('ticketWithDiscount', {
          MainDiscount: { percentage: '$root.price', $: 'discountSaved' }
        })
        .then((ticket) => {
          ticket.should.have.property('MainDiscount');
          ticket.MainDiscount.should.have.property('percentage', ticket.price);
          ticket.$.should.have.property('discountSaved');
          ticket.$.discountSaved.should.have.property(
            'id',
            ticket.MainDiscount.id
          );
        });
    });
  });
  ...
  describe('hasMany', function () {
    it('should define a sale asociated to many tickets', () => {
      ...
      const assocManyAfterConfig =
        factory.definitions.saleFactWithTickets.config.assocManyAfter;
      assocManyAfterConfig.should.have.property('Tickets');
      assocManyAfterConfig.Tickets.should.have.property(
        'factoryName',
        'ticketFactNoSale'
      );
      assocManyAfterConfig.Tickets.should.have.property('options', [
        { price: 120 / 2 },
        { price: 120 / 2 }
      ]);
      assocManyAfterConfig.Tickets.should.have.property(
    ...
    it('should create has many association', () => {
      return factory.create('saleB').then((ticket) => {
    ...
    it('should create has many with options', () => {
      return factory
        .create('saleB', { Tickets: { _size: 3, price: '$root.total' } })
        .then((sale) => {
    ...
    it('should respond error if id passed', () => {
      return factory
        ...
        .catch((err) => {
          err.message.should.equal(
            'Ids can not be passed in Tickets since it is a HasOne or HasMany association.'
          );
        });
    });
